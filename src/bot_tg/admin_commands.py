...
async def admin_logs_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        logs = get_request_logs(limit=100, date_from=date_start, date_to=date_end)
...
async def send_message_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=target_user_id, text=f"📨 <b>Сообщение от администратора:</b>\n\n{message_text}", parse_mode="HTML"
        )
        ...
        logger.info(
            f"📨 Админ {username} (ID: {user_id}) отправил сообщение пользователю {target_user_id}: {message_text}"
        )
        ...
        if "chat not found" in error_msg.lower():
            ...
            await update.message.reply_text(f"❌ <b>Ошибка отправки сообщения</b>\n\n" f"Ошибка: {error_msg}")
...
async def activate_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if success:
            ...
            logger.info(
                f"✅ Админ {username} (ID: {user_id}) активировал пользователя {target_username} (ID: {target_user_id})"
            )
        else:
            await update.message.reply_text(
        ...
        await update.message.reply_text(f"❌ <b>Ошибка активации пользователя</b>\n\n" f"Ошибка: {str(e)}")
...
async def deactivate_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if success:
            ...
            logger.info(
                f"🚫 Админ {username} (ID: {user_id}) деактивировал пользователя "
                f"{target_username} (ID: {target_user_id})"
            )
        else:
            await update.message.reply_text(
        ...
        await update.message.reply_text(f"❌ <b>Ошибка деактивации пользователя</b>\n\n" f"Ошибка: {str(e)}")
