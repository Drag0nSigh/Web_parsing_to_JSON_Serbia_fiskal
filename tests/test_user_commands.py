...
def mock_env_vars():
    ...
    with patch.dict(
        os.environ, {"TG_TOKEN": "123456789:TEST_TOKEN", "ADMIN_ID": "123456789", "DAILY_REQUEST_LIMIT": "50"}
    ):
        yield
...
try:
    from bot_tg.user_commands import admin_message, handle_message, help_command, is_url, start
...
class TestIsUrl:
    ...
    def test_is_url_with_path(self):
        ...
        assert is_url(test_url) is True
...
class TestAdminMessage:
    ...
    async def test_admin_message_daily_limit_exceeded(self, mock_update, mock_context):
        ...
        with patch("bot_tg.user_commands.is_user_active", return_value=True):
            with patch("bot_tg.user_commands.check_daily_limit") as mock_check_limit:
                mock_check_limit.return_value = {"can_make_request": False, "current_count": 50, "limit": 50}
