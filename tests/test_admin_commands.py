...
try:
    from bot_tg.admin_commands import (
        format_datetime, is_admin, admin_start, admin_logs, admin_logs_date,
        admin_users, admin_test, admin_status, admin_stats, send_message_to_user,
        activate_user_command, deactivate_user_command
    )
        ...
        with patch('bot_tg.admin_commands.get_system_stats') as mock_get_stats:
            with patch('bot_tg.admin_commands.get_database_info') as mock_get_db_info:
                mock_get_stats.return_value = {
                    ...
                    'unique_users': 10,
                    'daily_stats': [
                        {'date': '2025-09-28', 'total_requests': 20, 'unique_users': 5}
                    ]
                }
